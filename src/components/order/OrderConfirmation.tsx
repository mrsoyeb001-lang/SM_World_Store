                ...
                {steps.map((s, i) => {
                  ...
                  return (
                    <div key={s.key} className="flex flex-col items-center text-center">
                      <div className={`size-10 rounded-full flex items-center justify-center border-2 ${done ? "border-emerald-500 bg-emerald-50" : "border-muted"}`}>
                        <Icon className={`h-5 w-5 ${done ? "text-emerald-600" : "text-muted-foreground"}`} />
                      </div>
                      <div className={`mt-2 text-xs ${done ? "text-foreground font-medium" : "text-muted-foreground"}`}>{s.label}</div>
                    </div>
                  );
                })}
              </div>
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {settings?.support?.phone && (
                      <Button asChild variant="outline" className="rounded-2xl"><a href={`tel:${settings.support.phone}`}><Phone className="mr-2 h-4 w-4"/> কল</a></Button>
                    )}
            ...
            <CardContent className="py-6 text-xs text-muted-foreground">
              <div className="flex flex-col md:flex-row items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1"><Shield className="h-3.5 w-3.5"/>সিকিউর পেমেন্ট</div>
                </div>
              </div>
            </CardContent>
          </Card>
